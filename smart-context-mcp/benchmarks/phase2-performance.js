...
async function measurePerformance(name, fn, iterations, thresholdMs) {
    const times = [];
    // Warmup
    for (let i = 0; i < 3; i++) {
        await fn();
...
async function benchmarkAstDiff() {
...
export interface Config {
    ...
    `.trim();
    return measurePerformance('AstDiffEngine.diff() - typical file', async () => {
        await engine.diff('test.ts', oldCode, newCode);
    }, 100, 50 // < 50ms threshold
    );
}
...
async function benchmarkCallGraph() {
...
async function benchmarkSymbolImpact() {
    const engine = new AstDiffEngine();
    const oldCode = `
export class Calculator {
    add(a: number, b: number): number {
...
export class Calculator {
    ...
    return await measurePerformance('SymbolImpactAnalyzer (AST diff core) - class change', async () => {
        await engine.diff('calculator.ts', oldCode, newCode);
    }, 100, 200 // < 200ms threshold
    );
}
async function benchmarkAstDiffLargeFile() {
    const engine = new AstDiffEngine();
    // Generate large file (100 functions)
    const generateCode = (start, count) => {
        const functions = [];
...
export function func${i}(a: number, b: number): number {
    ...
    return measurePerformance('AstDiffEngine.diff() - large file (100 functions)', async () => {
        await engine.diff('large.ts', oldCode, newCode);
    }, 50, 100 // Allow more time for large files
    );
}
